...
async def die_(message: Message) -> None:
    ...
    system.Dynamic.RUN_DYNO_SAVER = asyncio.get_event_loop().create_task(
        _dyno_saver_worker()
    )
...
@userge.on_cmd(
    ...
    about={
        ...
        "flags": {"-c": "provide code", "-sc": "provide two step authentication code"},
...
async def convert_usermode(msg: Message):
    ...
    if msg.flags:
        if not hasattr(generate_session, "phone_number"):
            return await msg.err("first give phone number, click on below button 👇")
        ...
        if code:
            try:
                ...
                if await generate_session():
                    session_string = (
                        await generate_session.client.export_session_string()
                    )
...
@userge.on_cmd(
    ...
    about={
        ...
        "usage": "{tr}convert_botmode bot_name | bot_username",
    },
    allow_channels=False,
)
