...
async def helpme(
    ...
    if not message.input_str:
        ...
        if (
            ...
            and (
                len(plugins[key].loaded_commands) > 1
                or plugins[key].loaded_commands[0].name.lstrip(config.CMD_TRIGGER)
                != key
            )
        ):
...
if userge.has_bot:
    ...
    async def callback_chgclnt(callback_query: CallbackQuery):
        ...
        await SAVED_SETTINGS.update_one(
            {"_id": "CURRENT_CLIENT"},
            {"$set": {"is_user": config.Dynamic.USER_IS_PREFERRED}},
            upsert=True,
        )
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            ...
                            InlineKeyboardButton(
                                "🖥 Deploy Userge", url="https://t.me/theUserge/102"
                            ),
                        ]
                    ]
                ),
