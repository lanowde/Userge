...
@userge.on_cmd(
    ...
    about={
        ...
        "examples": ["{tr}s wel", "{tr}s -p gdrive"],
    },
...
async def search(message: Message):
    ...
    data = list(
        userge.manager.loaded_plugins if plugins else userge.manager.loaded_commands
    )
...
@userge.on_cmd(
    ...
    about={
        ...
        "flags": {"-h": "get heroku logs (default limit 100)", "-l": "get loader logs"},
        "examples": ["{tr}logs", "{tr}logs -h", "{tr}logs -h200", "{tr}logs -l"],
    },
    allow_channels=False,
)
async def check_logs(message: Message):
    ...
    if "-h" in message.flags and config.HEROKU_APP:
        ...
        await message.client.send_document(
            chat_id=message.chat.id, document=f"logs/{filename}", caption=filename
        )
