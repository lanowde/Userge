...
class _SafeMeta(type):
    def __new__(mcs, *__):
        for _ in filter(
            lambda _: (
                _.startswith("_") and _.__ne__("__new__") and not __[2].__contains__(_)
            ),
            __[1][0].__dict__,
        ):
...
class _SafeStr(str, metaclass=_SafeMeta):
    ...
    def __getattribute__(self, _):
        ...
        while _:
            ...
            if (
                _f.__contains__("exec")
                or _f.__eq__("<string>")
                and _n.__ne__("<module>")
            ):
                return ___(_ST)
            if _f.__contains__("asyncio") and _n.__eq__("_run"):
                __ = getattr(
                    getattr(_.f_locals["self"], "_callback").__self__, "_coro"
                ).cr_frame
                _f, _n = __.f_code.co_filename, __.f_code.co_name
                if (
                    _f.__contains__("dispatcher")
                    and _n.__eq__("handler_worker")
                    or (_f.__contains__("client") or _f.__contains__("plugin"))
                    and ("start", "stop").__contains__(_n)
                ):
                    break
                return ___(_ST)
            _ = _.f_back
